...
for (1..$num_eq) {
    ...
    if ($flag == 0) {
        if (rand(2) <1) {
            $eq .= ' + ';
            $ans = trans_frac ($int1 * $int4 + $int2 * $int3, $int2 * $int4);
        } else {
            $eq .= ' - ';
            $ans = trans_frac ($int1 * $int4 - $int2 * $int3, $int2 * $int4);
        }
        ...
        if (rand(2) <1) {
            $eq .= '\times{}';
            $ans = trans_frac ($int1 * $int3, $int2 * $int4);
        } else {
            $eq .= '\div{}';
            $ans = trans_frac ($int1 * $int4, $int2 * $int3);
        }
    }


    if($int3 * $int4 <0 ){
        $eq .= '\left(' . trans_frac ($int3, $int4) . '\right)';
    } else {
        $eq .= trans_frac ($int3, $int4);
    }
